package com.easybuy.controlador;

import com.easybuy.config.Fecha;
import com.easybuy.modelo.Carrito;
import com.easybuy.modelo.Cliente;
import com.easybuy.modelo.ClienteDAO;
import com.easybuy.modelo.Compra;
import com.easybuy.modelo.ComprasDAO;
import com.easybuy.modelo.DetalleCompra;
import com.easybuy.modelo.Pago;
import com.easybuy.modelo.Producto;
import com.easybuy.modelo.ProductoDAO;
...
//arquitectura modelo vista controlador(mvc)
   ...
   // int idm;
    ...
    //captura la sesion del usuario
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession session = request.getSession();
        session.setAttribute("logueo", logueo);//parametro de contraseña, al ser atributo el ingreso debe ser exacto a lo que esta registrado
        session.setAttribute("correo", correo);//parametro de correo, al ser atributo el ingreso debe ser exacto a lo que esta registrado
        productos = pdao.listar();
        
        String accion = request.getParameter("accion");

        switch (accion) {
            //trae la lista de productos que se vayan seleccionando y calcula el total a pagar
            case "carrito":
                totalPagar = 0.0;
                item = 0;
            ...
            // devuelve la lista de los productos agregados
            case "Comprar":
                agregarCarrito(request);
                request.getRequestDispatcher("Controlador?accion=carrito").forward(request, response);
                break;
             
             //ingresa los productos seleccionados y aumenta el contador
            case "AgregarCarrito":
                agregarCarrito(request);
            ...
            case "deleteProducto":
                ...
                break;
            //actualiza la cantidad de los productos en el carrito, se actualiza si refrescamos la pagina
            case "updateCantidad":
                idProducto = Integer.parseInt(request.getParameter("id"));
            ...
            //valida el mail y la contraseña coinciden con el usuario que este registrado en la base de datos
            case "Validar":
            ...
            case "Registrar":
             ...
             //trae todos los productos que tengo en la base de datos
            case "Nuevo":
                listaProductos = new ArrayList();
                request.getRequestDispatcher("Controlador?accion=home").forward(request, response);
                break;
            
            //rastrea en la base de datos el nombre ingresado de producto
            case "Buscar":
            ...
            case "GenerarCompra":
                ...
                if (cl.getId() != 0 && listaProductos.size() != 0 && montopagar > 0) {
                    if (totalPagar > 0.0) {
                        ...
                        co.setIdPago(idpago);
                        co.setEstado("Pagado - En Proceso de Envio");
            ...
            case "GuardarProducto":
                ...
                try {
                    //sirven para almacenar las imagenes
                    ...
                    p.setNombres(pro.get(0));//pro es un arraylist de tipo producto
                    p.setDescripcion(pro.get(1));
                    p.setPrecio(Double.parseDouble(pro.get(2)));
                    p.setStock(Integer.parseInt(pro.get(3)));
                    pdao.AgregarNuevoProducto(p);//llama a ProductoDAO

                } catch (Exception e) {
                    System.err.println(""+e);
                }
                request.getRequestDispatcher("Controlador?accion=NuevoProducto").forward(request, response);//si la operacion es correcta actualiza el nuevo producto
                break;
                
           case "EliminarProducto":
                codprod = Integer.parseInt(request.getParameter("id"));
                pdao.delete(codprod);
